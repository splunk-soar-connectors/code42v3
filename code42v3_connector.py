...
from code42v3_consts import (
    CODE42V3_CONNECTIVITY_ENDPOINT,
    CODE42V3_ERR_CONNECTIVITY_TEST,
    CODE42V3_SUCC_CONNECTIVITY_TEST,
)


class Code42v3Connector(BaseConnector):
    """
    Code42v3 connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = CODE42V3_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(CODE42V3_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(CODE42V3_SUCC_CONNECTIVITY_TEST)
